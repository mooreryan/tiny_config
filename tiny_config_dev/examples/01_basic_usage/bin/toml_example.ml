...
module Config = struct
  ...
  (* Parse the file path into an Otoml.t, or fail. *)
  let otoml_of_file name =
    match Otoml.Parser.from_file_result name with
    | Ok config ->
        config
    | Error msg ->
        failwith msg
  ...
  let of_file_name name = Tiny_toml.Term.eval term ~config:(otoml_of_file name)
end
...
let parse_argv () =
  ...
  | [|_; config|] ->
      config
  | _ ->
      failwith "usage: toml_example.exe <config.toml>"
